...
export async function renderTrader(view, id) {
  ...
  <div class="sec">
    <div class="mgrid card">
      ${metric(T('tr.strategyret'), sp(t.ret, 1), t.retOwn !== null ? T('p.ourtwr', { x: F.pct(t.retOwn, 1) }) : '')}
  ...
  <div class="s-nums">
    <div class="s-num">
      ...
      <div class="x">1 000 \u20ac \u2192 ${t.net === null ? '\u2014'
        : Math.round(1000 * (1 + t.net / 100)).toLocaleString('en-US').replace(/,/g, ' ') + ' \u20ac'}</div>
    </div>
    <div class="s-num">
      <div class="k">${T('scol.lost')}</div>
      <div class="v neg">${F.pct(t.dd, 0) ?? '\u2014'}</div>
      <div class="x">1 000 \u20ac \u2192 ${t.dd === null ? '\u2014'
        : Math.round(1000 * (1 + t.dd / 100)).toLocaleString('en-US').replace(/,/g, ' ') + ' \u20ac'}</div>
    </div>
    <div class="s-num">
