...
const D = {
  ...
  'p.origin':    ['Звідки взявся баланс', 'Откуда взялся баланс', "Da dove viene il saldo"],
  'p.fromtrade': ['від торгівлі', 'от торговли', 'dal trading'],
  'p.fromflow':  ['введено ззовні (внески й отримані комісії)',
                  'внесено извне (взносы и полученные комиссии)',
                  "entrato dall'esterno (versamenti e commissioni incassate)"],
  'p.withdrawn': ['виведено', 'выведено', 'prelevato'],
  'p.originnote':['Опублікований відсоток ці рухи <b>не рахує</b>: перевірено — якщо порахувати їх як прибуток, перерахунок не сходиться в жодного трейдера. Але сам баланс їх містить, тому великий рахунок не означає добру торгівлю.',
                  'Опубликованный процент эти движения <b>не считает</b>: проверено — если посчитать их как прибыль, пересчёт не сходится ни у одного трейдера. Но сам баланс их содержит, поэтому крупный счёт не означает хорошую торговлю.',
                  "La percentuale pubblicata <b>non li conta</b>: verificato — contandoli come guadagno il ricalcolo non torna su nessun trader. Ma il saldo li contiene, quindi un conto grande non significa aver operato bene."],
  'p.flowunknown': ['Історія операцій неповна, тож результат торгівлі виміряти не можна: показані лише рухи капіталу.',
                    'История операций неполная, поэтому результат торговли измерить нельзя: показаны только движения капитала.',
                    "Lo storico delle operazioni è parziale, quindi il risultato di trading non è misurabile: qui sotto ci sono solo i movimenti di capitale."],
  'flag.flow':   ['Баланс виріс переважно за рахунок зовнішніх надходжень, а не торгівлі',
                  'Баланс вырос в основном за счёт внешних поступлений, а не торговли',
                  'Il saldo è cresciuto soprattutto per denaro entrato, non per il trading'],
